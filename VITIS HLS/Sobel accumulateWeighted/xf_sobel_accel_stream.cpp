...
void sobel_accel(stream_t& img_inp, stream_o& img_out, float alpha, int shift, int rows, int cols)
{
// clang-format off
    ...
    xf::cv::Mat<TYPE_3, HEIGHT, WIDTH, NPIX> _dstgx(rows, cols);
    xf::cv::Mat<TYPE_3, HEIGHT, WIDTH, NPIX> _dstgy(rows, cols);
    xf::cv::Mat<TYPE_2, HEIGHT, WIDTH, NPIX> _dst(rows, cols);
    xf::cv::Mat<TYPE_3, HEIGHT, WIDTH, NPIX> dst_1(rows, cols);
    ...
    xf::cv::Sobel<XF_BORDER_CONSTANT, FILTER_WIDTH, TYPE, TYPE_3, HEIGHT, WIDTH, NPIX, false>(in_mat, _dstgx,_dstgy);
    ...
    // accumulate weighted
    xf::cv::accumulateWeighted<TYPE_3, TYPE_2, HEIGHT, WIDTH, NPIX>(_dstgx, _dstgy, _dst, alpha);


    // bit depth convert from TYPE_3 XF_16SC1 to TYPE_2 XF_8UC1
    xf::cv::convertTo<TYPE_2, TYPE_3, HEIGHT, WIDTH, NPIX>(_dst, dst_1, CONVERT_TYPE, shift);
    //printf("xfMat2Array .... !!!\n");
