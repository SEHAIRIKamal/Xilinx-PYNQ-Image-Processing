...
void XSobel_accel_Set_alpha(XSobel_accel *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XSobel_accel_WriteReg(InstancePtr->Control_BaseAddress, XSOBEL_ACCEL_CONTROL_ADDR_ALPHA_DATA, Data);
}

u32 XSobel_accel_Get_alpha(XSobel_accel *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XSobel_accel_ReadReg(InstancePtr->Control_BaseAddress, XSOBEL_ACCEL_CONTROL_ADDR_ALPHA_DATA);
    return Data;
}
