...
typedef struct {
    ...
    XMedian_blur_accel_uio_map maps[ MAX_UIO_MAPS ];
} XMedian_blur_accel_uio_info;

/***************** Variable Definitions **************************************/
static XMedian_blur_accel_uio_info uio_info;
...
static int uio_info_read_name(XMedian_blur_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMedian_blur_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMedian_blur_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMedian_blur_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMedian_blur_accel_Initialize(XMedian_blur_accel *InstancePtr, const char* InstanceName) {
	XMedian_blur_accel_uio_info *InfoPtr = &uio_info;
