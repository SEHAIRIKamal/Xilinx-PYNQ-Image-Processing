...
typedef struct {
    ...
    XModefilter_accel_uio_map maps[ MAX_UIO_MAPS ];
} XModefilter_accel_uio_info;

/***************** Variable Definitions **************************************/
static XModefilter_accel_uio_info uio_info;
...
static int uio_info_read_name(XModefilter_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XModefilter_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XModefilter_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XModefilter_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XModefilter_accel_Initialize(XModefilter_accel *InstancePtr, const char* InstanceName) {
	XModefilter_accel_uio_info *InfoPtr = &uio_info;
