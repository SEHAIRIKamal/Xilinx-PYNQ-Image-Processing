...
typedef struct {
    ...
    XCvtcolor_rgb2gray_uio_map maps[ MAX_UIO_MAPS ];
} XCvtcolor_rgb2gray_uio_info;

/***************** Variable Definitions **************************************/
static XCvtcolor_rgb2gray_uio_info uio_info;
...
static int uio_info_read_name(XCvtcolor_rgb2gray_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCvtcolor_rgb2gray_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCvtcolor_rgb2gray_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCvtcolor_rgb2gray_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCvtcolor_rgb2gray_Initialize(XCvtcolor_rgb2gray *InstancePtr, const char* InstanceName) {
	XCvtcolor_rgb2gray_uio_info *InfoPtr = &uio_info;
