...
typedef struct {
    ...
    XScharr_accel_uio_map maps[ MAX_UIO_MAPS ];
} XScharr_accel_uio_info;

/***************** Variable Definitions **************************************/
static XScharr_accel_uio_info uio_info;
...
static int uio_info_read_name(XScharr_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XScharr_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XScharr_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XScharr_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XScharr_accel_Initialize(XScharr_accel *InstancePtr, const char* InstanceName) {
	XScharr_accel_uio_info *InfoPtr = &uio_info;
